    ...
    try {
      if (isPatient && user?.pacienteId) {
        ...
        setStats({
          ...
          completed: appointments.filter((a) => a.estado === "COMPLETED")
            .length,
        });
        setUpcoming(
          appointments
            .filter((a) => a.estado === "SCHEDULED" || a.estado === "CONFIRMED")
            .slice(0, 5),
        );
      } else {
      ...
      >
        ...
        >
          {user?.fotoPerfil ? (
            <Image
              source={{
                uri: `${API_BASE_URL}/profile/photo/${user.fotoPerfil}`,
              }}
...
*/}
      ...
      {upcoming.length === 0 ? (
        ...
        upcoming.map((appt) => {
          const statusColor =
            colors[STATUS_COLOR_KEY[appt.estado]] ?? colors.textMuted;
          return (
            <View
              key={appt.id}
              style={[
            ...
            >
              ...
              >
                <Text style={[styles.dateDay, { color: colors.primary }]}>
                  {new Date(appt.fechaHora).getDate()}
                </Text>
                <Text style={[styles.dateMon, { color: colors.primary }]}>
                  {new Date(appt.fechaHora)
              ...
              <View style={styles.apptInfo}>
                <Text
                  style={[styles.apptName, { color: colors.textPrimary }]}
                  numberOfLines={1}
                >
                ...
                <View style={styles.apptMeta}>
                  ...
                  <Text style={[styles.apptTime, { color: colors.textMuted }]}>
                    {new Date(appt.fechaHora).toLocaleTimeString("es", {
                      hour: "2-digit",
                      minute: "2-digit",
                    })}
                  </Text>
